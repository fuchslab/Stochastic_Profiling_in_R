...
# Predict Cell Combos
###############################################################################################
# CI done here only works properly for 2Pop
  ...
  True.predictions.MLE_CI_wp[i] <-sum(CI_l_wp[[i]][,1]<= N.matrix[,1] &  N.matrix[,1]<= CI_u_wp[[i]][,1])     # only for 2Pop
  True.predictions.MLE.orig_wp[i] <- sum(MLE.orig_wp[[i]]== N.matrix)/ncol(N.matrix)
  True.predictions.MLE_CI.orig_wp[i] <-sum(CI.orig_l_wp[[i]][,1]<= N.matrix[,1] &  N.matrix[,1]<= CI.orig_u_wp[[i]][,1])     # only for 2Pop
...
well.prediction(dataset = Dataset_1, k = k_1, n = n_1, p.vector = p.vector_1, mu.vector = mu.vector_1, 
                sigma.vector = sigma.vector_1, N.matrix = N.matrix_1, set_a_seed = set_seeds_1[2] + 102, save_name ="Dataset_1_all.rda")
...
##############################################################################################
# Calculate all predictions etc and save
well.prediction(dataset = Dataset_2, k = k_2, n = n_2, p.vector = p.vector_2, mu.vector = mu.vector_2,
                sigma.vector = sigma.vector_2, N.matrix = N.matrix_2, set_a_seed = set_seeds_2[2] + 102,
                save_name = "Dataset_2_all.rda")
