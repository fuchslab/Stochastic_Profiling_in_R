...
for(whichSet in c("Set1", "Set2", "Set3", "Set4", "Set5")){
 ...
 for(j in pool_sizes){
  ...
  for (i in 1:nn){
    ...
    result_fit_list[[i]] <- stochprof.loop(model = "LN-LN", dataset = dataset, n = n, TY = TY, genenames = genes, fix.mu = FALSE, loops = 10,
                                           until.convergence = FALSE, print.output = FALSE, show.plots = FALSE, plot.title = "Entire Cluster", use.constraints = FALSE)
    sink()
